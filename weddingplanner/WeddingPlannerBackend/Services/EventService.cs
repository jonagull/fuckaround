...
public class EventService : IEventService
{
  ...
  public async Task<ResponseEvent> UpdateEventAsync(Guid eventId, Guid userId, RequestUpdateEvent request)
  {
    ...
    var canEditEvent = eventEntity.Planners.Any(p => p.UserId == userId && (p.Role == EventRole.OWNER || p.Role == EventRole.ADMIN));

    if (!canEditEvent) throw new UnauthorizedAccessException("Only event owners can update events");
    ...
    if (request.EventDate.HasValue) eventEntity.EventDate = request.EventDate;
